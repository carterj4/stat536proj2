...
Companies are invested in having productive employees. Clearly increased employee productivity increases a companies profits. Generally it is thought that happy employees will be more productive, but how much does employee hapiness actually affect job performance? 
...
In order to evaluate how well the model performs on the data, we will report the $R^2$ value, the variance of the model coefficients between imputed datasets, and the trace plots. The average $R^2$ value for this model is \Sexpr{round(mean(r2),3)} which is decent. The variance of the estimated JobSat coefficients is \Sexpr{round(var(js.b),4)} and the variance of the estimated WellBeing coefficients is \Sexpr{round(var(wb.b),4)}, which are both very small. This indicates that the missing values are not having much of an effect on the estimated coefficients. Trace plots are used to make sure that the coefficients have converged to some region. It is possible that, as the imputed datasets are generated, the values of the estimated coefficients keep increasing or decreasing. Figures \ref{fig:js} and \ref{fig:wb} show that the values have stabilized in an acceptable region, making our estimates valid.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% also include trace plots. Decide later how to name variables in paper
\section{Results}

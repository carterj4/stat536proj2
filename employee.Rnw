...
=======
At the end of the day, we want to be able to quantify the relationship between job performance and the other variables, to see if happiness really influences work performance. Finding a thoughtful way to deal with missing data will be key because the data are missing in the variables we are most interested in. Missing data can be difficult to deal with because the reason for missingness is often unknown. Data that are missing at random can be dealt with by simply using the complete observations; however, data not missing at random can mask or obscure the true effect of a variable. \\
\indent We now investigate properties of the missing observations to see if the missing values appear to be random. We treated the missingness of well-being as a categorical variable in order to compare the distribution of each variable by whether well-being is missing or not. The procedure was repeated for job satisfaction and job performance. We found that, when job performance was missing, both job satisfaction and well-being were lower. In particular, the maximum score for job satisfaction when job performance was missing was lower than the first quartile for job satisfaction when job performance was not missing.  This indicates there is non-randomness in the missing data, which means it would be extremely unreliable to use only the complete observations for our analysis. Tables \ref{tab:ps} and \ref{tab:pw} below show the different summaries. In the section \ref{methods} we further discuss the imputation method used to correct for this.
>>>>>>> 847f92e8798f955e721201b19e601516811745ae
...
\begin{tabular}{rrr}
\hline
$R^2$ & \texttt{JobSat} Var & \text{WellBeing} Var \\
\hline
...
Table \ref{tab:coefs} shows the estimated coefficients of job satisfaction and well-being from fitting the model as well as the 95\% confidence interval bounds. Notice that the interval for well-being does not contain zero and is thus significant. This means that for every point higher that someone rates their overall well-being, their job performance increases by almost half a point. In simpler terms, it appears that higher well-being is related to higher job performance, however it appears that job satisfaction does not affect performance.

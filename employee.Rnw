...
Proceeding with multiple linear regression using only the complete observations in the dataset would effectively throw out 72\% of the observations. This is an extreme loss of information. 
...
In order to evaluate how well the model performs on the data, we will report the $R^2$ value, the variance of the model coefficients between imputed datasets, and the traceplots. The average $R^2$ value for this model is \Sexpr{round(mean(r2),3)}. The variance of the estimated JobSat coefficients is \Sexpr{round(var(js.b),4)} and the variance of the estimated WellBeing coefficients is \Sexpr{zapsmall(var(wb.b),4)}, which are 

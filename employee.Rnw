...
\end{figure}
Graphically the data appear to be easy to work with, however there are many missing data points for wellb being, job satisfaction and  job performance. Of the 480 observed employee responses, \Sexpr{miss <- sum(apply(emp,1,FUN = function(x) any(is.na(x)))); miss} or \Sexpr{round(miss/480*100,0)}\% are missing at least one variable. Table \ref{tab:miss} shows the number of missing values in each variable and table \ref{tab:cmiss} shows the total number of observations missing x number of variables. 

...
\begin{abstract}
<<<<<<< HEAD
This is a generic abstract. FIX ME!
%\keywords{}
=======
% We would like to encourage you to list your keywords within
% the abstract section using the \keywords{...} command.
\keywords{}
>>>>>>> fab19a0e225cb048e4dfe6632be8e6f3cce5e26e
\end{abstract}
...
\end{figure}
Graphically the data appear to be easy to work with, however there is a problem with missing data. Of the 480 observed employee responses, \Sexpr{miss <- sum(apply(emp,1,FUN = function(x) any(is.na(x)))); miss} or \Sexpr{round(miss/480*100,0)}\% are missing at least one variable. Missing data can be difficult to deal with because the reason is often unknown. Data that are missing at random can be dealt with using imputation techniques, however data that are missing due to a cause significant to the study can mask or obscure what may have otherwise been obvious results. We now investigate properties of the missing observations to see if the missing values appear to be random.\\
\indent Table \ref{tab:miss} shows the number of missing values in each variable and table \ref{tab:cmiss} shows the total counts of the number of missing values.
...
$$
If we partition the observation vector into $\Y_1$ and $\Y_2$ and the covariance matrix we can find the joint conditional distribution of $\Y_1|\Y_2$. 
$$
...
\mathbf{\Sigma}_{1|2}
\end{align*}
Using the MVN distribution will allow us to impute the missing values in our dataset and then obtain estimates for the effect of job satisfaction and well being on job performance. Repeating the imputations multiple times will allow us to explore a whole range of possible values for the missing datapoints and thus see how different datasets affect our estimate. 
